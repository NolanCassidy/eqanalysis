...
# global data for map - if we had learned about classes yet, this would have
...
def plot_clusters(eq_clusters, eq_dict):
    ...
    """
    global eq_turtle
    colors = ['red', 'violet', 'blue', 'orange', 'cyan', 'yellow']

    for index in range(NO_OF_CLUSTERS):
        eq_turtle.color(colors[index])
        for key in eq_clusters[index]:
            lon = eq_dict[key][0]
            lat = eq_dict[key][1]
...
def plot_magnitudes(eq_dict):
    ...
    colors = ['indigo', 'blue', 'violet', 'cyan', 'green', 'yellow', 'orange', 'red']

    for key in eq_dict:
        ind = bin_value(eq_dict[key][2], bounds)
        if ind >= len(colors):
            ind = len(colors) - 1
        lon = eq_dict[key][0]
        lat = eq_dict[key][1]
        x,y = xy_calculate(lon, lat)
        eq_turtle.goto(x, y)
        eq_turtle.dot(5*(ind+1), colors[ind])
...
def plot_depths(eq_dict):
    ...
    """
    global eq_turtle
    bounds = [1.0, 5.0, 10.0, 20.0, 30.0, 50.0, 100.0]
    colors = ['violet', 'blue', 'cyan', 'green', 'yellow', 'orange', 'red']

    for key in eq_dict:
        ind = bin_value(eq_dict[key][3], bounds)
        if ind >= len(colors):
            ind = len(colors) - 1
        lon = eq_dict[key][0]
        lat = eq_dict[key][1]
...
def analyze_depths(eq_dict):
    ...
    depths = []
    for key in eq_dict:
        depths.append(eq_dict[key][3])
    mean,variance = data_mean_variance(depths)
    print("Analysis of depth data")
    print('  Mean depth = {:.1f} miles'.format(mean))
    print('  Median depth = {:.1f} miles'.format(data_median(depths)))
    print('  Standard deviation = {:.2f} miles'.format(math.sqrt(variance)))
...
def analyze_magnitudes(eq_dict):
    ...
    magnitudes = []
    for key in eq_dict:
        magnitudes.append(eq_dict[key][2])
    mean,variance = data_mean_variance(magnitudes)
    print("Analysis of magnitude data")
    print('  Mean magnitude = {:.1f}'.format(mean))
    print('  Median magnitude = {:.1f}'.format(data_median(magnitudes)))
    print('  Standard deviation = {:.2f}'.format(math.sqrt(variance)))
...
def analyze_clusters(eq_clusters, eq_dict):
    ...
    Outputs:
        for each cluster:
            ...
            mean, median, and standard deviation of depth data
    """
    for cl in range(len(eq_clusters)):
        depths = []
        magnitudes = []
        for i in eq_clusters[cl]:
            depths.append(eq_dict[i][3])
            magnitudes.append(eq_dict[i][2])
        print("Analysis of cluster", cl)
        mean,variance = data_mean_variance(magnitudes)
        print("  Analysis of magnitude data")
        print('    Mean magnitude = {:.1f}'.format(mean))
        print('    Median magnitude = {:.1f}'.format(data_median(magnitudes)))
